...
def assign_hand_detections_pose_based(detections, left_wrist_xy, right_wrist_xy):
    ...
    if len(detections) >= 2:
        best_sum = float('inf')
        best_pair = None
        # 두 검출 결과 중 왼손과 오른손을 결정하는 최적의 조합 찾기
...
# Mediapipe 초기화: Pose, Holistic, Hands 모듈 모두 사용
...
mp_drawing = mp.solutions.drawing_utils  # (손 시각화용)
...
while cap.isOpened():
    ...
    # 1. Pose 모듈로부터 손목 좌표 획득 (보간 및 할당 기준)
    ...
    # 3. Hands 검출 결과가 부족하면, Holistic 모듈의 손 랜드마크 보조 활용
    if len(detections) < 2:
    ...
    # 4. Pose 기반 손 할당: 여러 검출 결과 중 왼손/오른손 결정
    ...
    # 6. 손 검출 실패 시, Pose 손목 좌표의 이동량을 이용한 보간
    ...
    cv2.imshow("Hand Skeleton (Kalman Filter + Interpolation, Pose+Holistic+Hands)", frame)
